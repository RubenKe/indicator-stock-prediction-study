...
class DonchianBreakout(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.dc_exit_low = bt.ind.Lowest(self.data.low, period=self.p.exit_len)
    ...
    def next(self):
        ...
        upper_entry_prev = self.dc_entry_high[-1]
        lower_entry_prev = self.dc_entry_low[-1]
        upper_exit_prev = self.dc_exit_high[-1]
        lower_exit_prev = self.dc_exit_low[-1]
        atr_now = self.atr[0]
        ...
        if self.position.size > 0:
            ...
            self.lowest_close = min(self.lowest_close, close_now)

            stop_price = self.entry_price + self.p.stop_atr * self.entry_atr
            trail_stop = self.lowest_close + self.p.trail_atr * atr_now
            max_hold_hit = (len(self) - self.entry_bar) >= self.p.max_hold_bars

            if (
                close_now >= upper_exit_prev
                or close_now >= stop_price
                or close_now >= trail_stop
                or max_hold_hit
            ):
        ...
        if self.position.size == 0 and not self._in_cooldown():
            buffered_upper = upper_entry_prev + self.p.entry_buffer_atr * atr_now
